...
if symmetric_returns == 1
    ...
    if state_dependent_xi == 0
        ...
        filename = (compute_irfs==0) ? "model_sym_HS.npz" : "model_sym_HS_24_p.npz";
...
elseif symmetric_returns == 0
    ...
    if state_dependent_xi == 0
        println(" No tilting (xi is NOT state dependent)                      ")
        filename = (compute_irfs==0) ? "model_asym_HS.npz" : "model_asym_HS_24_p.npz";
    elseif state_dependent_xi == 1
        println(" With tilting (change in kappa)                        ")
...
if symmetric_returns == 1
    ...
    elseif state_dependent_xi == 2
        ...
        kappa_tilde    = kappa_hat
        alpha_k1_tilde = alpha_k1_hat
        beta1_tilde    = beta1_hat + .22
        alpha_k2_tilde = alpha_k2_hat
        beta2_tilde    = beta2_hat + .22
...
elseif symmetric_returns == 0
    ...
    if state_dependent_xi == 0
        ...
        ell_star = 0.026496299253771224

    elseif state_dependent_xi == 1
        # State-dependent tilting function (fixed kappa, alpha targets q)
        scale = 1.0205
        alpha_k2_hat = alpha_k1_hat = alpha_k_hat #+ .0028;
    ...
    elseif state_dependent_xi == 2
        # State-dependent tilting function (fixed beta1, alpha targets q)
        scale = 1.447
        alpha_k2_hat = alpha_k1_hat = alpha_k_hat + .0008;

        #alpha_z_tilde  = minimum_xi(-.0034, alpha_z_hat, sigma_z)
        alpha_z_tilde  = -.0034
        kappa_tilde    = kappa_hat
        alpha_k1_tilde = alpha_k1_hat
        beta1_tilde    = beta1_hat + .246
        alpha_k2_tilde = alpha_k2_hat
        beta2_tilde    = beta2_hat + .246
...
II_opt, JJ_opt = 1001, 201;     # number of r points, number of z points
rmax_opt = 18.;
rmin_opt = -rmax_opt;
zmax_opt = 1.2;
zmin_opt = -zmax_opt;
...
maxit = 350;        # maximum number of iterations in the HJB loop
crit  = 10e-6;      # criterion HJB loop
Delta = 1000.;      # delta in HJB algorithm
#Delta = 50.;      # delta in HJB algorithm
...
npzwrite("./result_files/24/" * filename, results)
