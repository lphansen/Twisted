...
for ki=1:length(kappa_list2)
  for qi=1:length(q_list)
     alpha_tilde, wc = choose_alpha_statistician(q_list[qi],kappa_list2[ki],
...
for ki=1:length(kappa_list2[2:end])
  for qi=1:length(q_list)
     ...
     beta_tilde, wc = choose_beta_statistician(q_list[qi], alpha_z_tilde, baseline.kappa_hat,
...
npzwrite("./result_files/28/model_singlecapital_statistician.npz", vcat(store_matrix, store_matrix2))
