...
class Model(object):
    ...
    def figure_1(self, cut=25, left_top_ylim=None, left_bottom_ylim=[-.0005, .0005], numb_lcurves=5, perc=.1):
        ...
        J_half = int(self.J/2)
        np.savez('./data/plotting_data/figure_1.npz',
                 ind_ld_z=ind_ld_z,
                 ind_med_z=ind_med_z,
                 ind_ud_z=ind_ud_z,
                 ind_med_r=ind_med_r,
                 ...
                 d1=d1,
                 d2=d2,
                 mu_pii_noR=mu_pii_noR,
                 mu_pii=mu_pii,
                 mu_pii_wc=mu_pii_wc,
                 f_noR_dist=f_noR_dist,
                 f_dist=f_dist,
                 f_wc_dist=f_wc_dist,
                 alpha_z_tilde=alpha_z_tilde,
        ...
        for kappa, alpha in models:
            if kappa == 0.169 and alpha == 0:
                ...
                if ite == 0: 
                    ...
                    fig.add_trace(go.Scatter(x = self.sets_const_tilt[kappa,alpha][:,0], y = self.sets_const_tilt[kappa,alpha][:,1], visible = True, showlegend = True, name = r'Boundary (iso-ϱ curve)', legendgroup = 'isocurve'))
                    ite = 1
                else:
                    fig.add_trace(go.Scatter(x = [kappa], y = [alpha], visible = False, name = "Worrisome Model", showlegend = True, legendgroup = 'Worrisome Model'))
                    fig.add_trace(go.Scatter(x = self.sets_const_tilt[kappa,alpha][:,0], y = self.sets_const_tilt[kappa,alpha][:,1], visible = False, showlegend = True, name = r'Boundary (iso-ϱ curve)', legendgroup = 'isocurve'))
...
def figure_1(cut=25, left_top_ylim=None, left_bottom_ylim=[-.0005, .0005], numb_lcurves=5, perc=.1):
    # load plotting data
    data = np.load('./data/plotting_data/figure_1.npz')
    
    ind_ld_z=data['ind_ld_z']
    ind_med_z=data['ind_med_z']
    ind_ud_z=data['ind_ud_z']
    ind_med_r=data['ind_med_r']
    pii=data['pii']
    zz=data['zz']
    d1_noR=data['d1_noR']
    d2_noR=data['d2_noR']
    d1=data['d1']
    d2=data['d2']
    mu_pii_noR=data['mu_pii_noR']
    mu_pii=data['mu_pii']
    mu_pii_wc=data['mu_pii_wc']
    f_noR_dist=data['f_noR_dist']
    f_dist=data['f_dist']
    f_wc_dist=data['f_wc_dist']
    alpha_z_tilde=data['alpha_z_tilde']
    kappa_tilde=data['kappa_tilde']
    J_half=data['J_half']
